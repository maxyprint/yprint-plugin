...
function yprint_get_email_template($title, $username, $content) {
    ...
    if (!$template) {
        ...
        // Fallback zur alten Methode mit HTML direkt im Code
        ob_start();
        ?>
        <!DOCTYPE html>
        <html lang="de">
    ...
    // Platzhalter ersetzen (behalte die Parameterbezeichnungen für Kompatibilität bei)
    $template = str_replace('{{headline}}', $title, $template);
    $template = str_replace('{{first_name}}', $username, $template);
